...
async def search_api_server():
    MAX_ATTEMPTS = 10  # Define a maximum number of search attempts
    attempt = 0
    global BASE_URL
    while not BASE_URL and attempt < MAX_ATTEMPTS:
        # Create a UDP socket
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            ...
            try:
                ...
                attempt += 1
                emit_socketio_event('vtstudio_error', {'message': f'Attempt {attempt}: No response from VTubeStudio API server'})

    if not BASE_URL:
        emit_socketio_event('vtstudio_error', {'message': 'Failed to find VTubeStudio API server after multiple attempts'})
